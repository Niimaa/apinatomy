...
/***/ function(module, exports, __webpack_require__) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(62), __webpack_require__(15), __webpack_require__(63), __webpack_require__(64), __webpack_require__(12)], __WEBPACK_AMD_DEFINE_RESULT__ = function ($, U, P, THREE, Kefir) {
		'use strict';
		...
		var tY = function tY(cb, y) {
			return cb.size.height - y - 2;
		}; // TODO: magic nr '2' has to do with borders/padding
		...
		function bezierMesh(cb, edge, color) {
			...
			result._amy_interface = {
				removeTube: function removeTube() {
					cb.object3D.remove(result);
				},
				updateTube: function updateTube() {
	
					var v3 = function v3(v) {
						return new THREE.Vector3(v.x, tY(cb, v.y), v.z);
					};
	
					/* control point distance */ // TODO: optimize for aesthetics (http://stackoverflow.com/q/30424772/681588)
					var cpd = { source: 0.45, target: 0.45 };
	
					var other = function other(d) {
						return d === 'source' ? 'target' : 'source';
					};
	
					var vForce3 = function vForce3(direction) {
						...
						if (edge[direction].value.location === 'inter-tile') {
							...
							try {
								for (var _iterator = edge[direction].verticesTo()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
									var pred = _step.value;
	
									var contribution = v3(pred).sub(v3(edge[direction].value.d3Vertex)).normalize();
									if (direction === 'source') {
										contribution.negate();
									}
							...
							try {
								for (var _iterator2 = edge[direction].verticesFrom()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
									var succ = _step2.value;
	
									var contribution = v3(succ).sub(v3(edge[direction].value.d3Vertex)).normalize();
									if (direction === 'target') {
										contribution.negate();
									}
									force.add(contribution);
					...
					var V = [v3(edge.source.value.d3Vertex), v3(edge.source.value.d3Vertex).add(vForce3('source')), v3(edge.target.value.d3Vertex).add(vForce3('target')), v3(edge.target.value.d3Vertex)];
					result.geometry.dispose();
					var curve = new (_bind.apply(THREE.CubicBezierCurve3, [null].concat(V)))();
...
;
//# sourceMappingURL=p-three-d-tubes.js.map